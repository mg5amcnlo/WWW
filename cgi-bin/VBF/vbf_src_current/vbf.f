      ...
      integer  itmx, nprn, ncall, ncall_warm
      ...
      double precision reg(18), reg_intf(12),reg_sch(6),reg_loop(12)
...
c-----if pdf=1 the pdf error is computed (only for xmu=1)      
      read(77,101) pdf
      read(77,101) iwdth !0: NWA, 1:CPS
      write(*,*) isch, icoll, sc,pdf, xmuf, xmur

      if (iproc.ne.1.and.iwdth.ne.0) then
          write(*,*) 'CPS only available for SM Higgs, doing NWA'
          iwdth=0
      endif
...
c   vegas warmup
      ilast=0
      call vegas(reg,9,integrand,0,ncall_warm,
     1        itmx,nprn,integral,error,prob)
...
c   vegas main call
      ...
      call vegas(reg,9,integrand,1,ncall,
     ...
     1 errpdf(sigtot(k),sigtotpdf,epdfs(k),epdfu(k),epdfd(k),pdfshort)
      ...
      write(80,120)iproc,icoll,iwdth,
...
111   format(2a5, 3a21)
120   format(3(i7),f10.2,f10.2,2(i3),2(f7.4),6(e15.6))
   
      stop
      end

      double precision function gethwdth(m)
* this sub returns the higgs boson width for a given mass/virtuality m
      implicit none 
      double precision m
      call gridHt(m, gethwdth)
      return
      end
...
*this sub computes pdf errors in the abkm scheme
      ...
      implicit none
      double precision par(9),x1,x2,q1,q2,qt1,phi,qt2,sqrts,EE
...
c      do vec=-1,1
       do v1=1,3
       do v2=1,3
       if(v1.eq.1.or.v1.eq.2) then 
           mv1=mw
           wv1=wwdth
       else if (v1.eq.3) then
           mv1=mz
           wv1=zwdth
       endif
       if(v2.eq.1.or.v2.eq.2) then 
           mv2=mw
           wv2=wwdth
       else if (v2.eq.3) then
           mv2=mz
           wv2=zwdth
       endif
...
c----end of v-loop
      ...
      implicit none
      double precision xx(9), ff, par(9),sqrts,dsig,pi,
     ...
     1 ,eps
      ...
      integer iwdth
      common/higgs_width/iwdth
      double precision decay,gethwdth,hvirt2, hvirt
      double precision hwdth_m, hwdth_q
      double precision mt, mb
      common/qmass/mt,mb
      data hwdth_m /-1.d0/
      logical firsttime
      data firsttime /.true./
      ! for the adaptive q1 integration
      double precision xi1, jacxi1, mv
      ...
      q1up=(sqrts-mh)**2-q0
c!new parameterization for q1
c do the q1 integration in an adaptive way
C result ~ dq / (q1 + A)**2, A = mv**2
c so generate  d xi  = -d (1/(q1 +A))
c xi ranges in 1/(A+q1up), 1/A, 
      mv = 81d0
      xi1 = 1d0/(mv**2+ q1up) + xx(1) *
     $  (1d0/(mv**2+q0) - 1d0/(mv**2+q1up))
      q1 = 1d0/xi1 - mv**2
      par(1) = q1
      if(q1.lt.q0.or.q1.gt.q1up.or.q0.gt.q1up) then
        write(*,*)'ERROR',q1, 'not in',q0,q1up
        do ipdf=1,npdf
        integralpdf(ipdf)=0d0
        enddo
        integrand=0d0
      ...
      jacxi1 = (1d0/(mv**2+q0) - 1d0/(mv**2+ q1up)) * (mv**2 + q1)**2
...
c     Narrow Width Approximation  
          ...
          hvirt = mh
          decay = 1d0
      elseif (iwdth.eq.1 .or. iwdth.eq.2 ) then
          decay = 1d0
c          hwdth_m = gethwdth(mh)
          if (firsttime) then
              call call_hto(mh, mt, hwdth_m)
              firsttime = .false.
          endif
          if (hwdth_m .lt. 0d0) then
              write(*,*) "Error, hwdth_m <0", hwdth_m
              stop
          endif
          hvirt2 = 
     1     max(0d0, mh**2 + hwdth_m * mh * dtan(pi *(-0.5d0 + xx(9))))
          hvirt = dsqrt(hvirt2)
c re compute x1 and x2          
          q1up=(sqrts-hvirt)**2-q0
          !new parameterization for q1
          xi1 = 1d0/(mv**2+ q1up) + xx(1) *
     $  (1d0/(mv**2+q0) - 1d0/(mv**2+q1up))
          q1 = 1d0/xi1 - mv**2
          par(1) = q1
          ...
          x1=dexp(-par(6))
          x2=tau/x1
          hwdth_q = gethwdth(hvirt)
          decay = hvirt * hwdth_q / mh / hwdth_m 
c         Standard BW approximation (see e.g. 1202.3638, eq 4.4)
          if (iwdth.eq.2) then 
c         this is for the running width (see e.g. 1202.3638, eq 4.5)  
            decay = decay 
     1        * ( (hvirt2 - mh**2)**2 + mh**2 * hwdth_m**2 )    
     1        / ( (hvirt2 - mh**2)**2 + hvirt2 * hwdth_q**2 )    
          endif
      else
          write(*,*) "Not valid value for iwdth ", iwdth
          stop
      endif
     ...
     1 *x1*x2-hvirt**2-q2-q1+ q1*q2/(sqrts**2 *x1*x2)
      s2=q2*(1d0/x2-1d0)
     ...
     1             abs(2d0*A*qt2b+B))
       ...
       do ipdf=1,npdf
       integralpdf(ipdf)=dsigpdf(ipdf)*jac*gevtopb*decay
       enddo

      integrand=dsig*jac *gevtopb*decay
